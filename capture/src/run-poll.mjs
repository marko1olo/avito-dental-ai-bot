...
import { SendFailed, sendMessage } from "./send.mjs";
import {
	checkSession,
	loadSelectors,
	MESSENGER_URL,
...
async function sendCycle(page, sel, db) {
	...
	for (const row of rows) {
		try {
			...
			const result = await sendMessage(page, sel, {
				chatIndex: 0,
				text: row.text,
			});
			...
			if (err instanceof SendFailed) {
				...
				log(
					`ОТПРАВКА #${row.id} ОБОРВАЛАСЬ, строка осталась в sending: ${err.message}`,
				);
...
function projectRoot() {
	...
	return path
		.resolve(path.dirname(new URL(import.meta.url).pathname), "..", "..")
		.replace(/^[/\\]([A-Za-z]:)/, "$1");
}
		...
		while (!stopping) {
			try {
				...
				if (!again || again.status !== "MESSENGER_RENDERED") {
					log(
						"сессия Авито мертва, выходим — нужен вход руками (npm run login)",
					);
					return 1;
				}
			}
