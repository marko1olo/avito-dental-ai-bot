...
async function probe(page, chatListSelector) {
	return page.evaluate((given) => {
		...
		return {
			...
			loginForm: count(
				'form[action*="login"], [data-marker*="login/"], [data-marker*="login-form"]',
			),
...
function looksLikeMessenger(url, signals) {
	...
	return (
		signals.channelMarkers + signals.channelLinks + signals.messengerShell > 0
	);
}
...
function printInstructions(profile, selectorsKnown) {
	...
	console.log(
		`Ожидание: до ${WAIT_MINUTES} мин (переопределяется AVITO_LOGIN_WAIT_MIN).`,
	);
	console.log("");
	console.log("Что сделать в открывшемся окне:");
	console.log(
		"  1. Войти в РАБОЧИЙ аккаунт клиники: телефон, пароль, код из SMS.",
	);
	console.log(
		"  2. Капчу, если попросит, решить руками. Скрипт в это не вмешивается.",
	);
	console.log("  3. Дойти до списка сообщений. Дальше ничего не делать —");
	console.log("     скрипт сам заметит мессенджер и продолжит.");
	console.log("");
	console.log(
		"Окно можно закрыть самому, когда вход выполнен: проверка всё равно",
	);
	console.log("пройдёт — она поднимает профиль заново, уже headless.");
	console.log(
		"Ничего в чатах писать НЕ надо: это живая переписка с пациентами.",
	);
	if (!selectorsKnown) {
		console.log("");
		console.log("capture/selectors.json пока нет — это ожидаемо до разведки.");
		console.log(
			"Вход проверяется по запасным признакам; структуру страницы потом",
		);
...
async function waitForHumanLogin(selectors) {
	...
	try {
		...
		await page
			.goto(MESSENGER_URL, { waitUntil: "domcontentloaded", timeout: 60_000 })
			.catch((err) =>
				console.log(`  (не удалось открыть мессенджер сразу: ${err.message})`),
			);
		...
		while (Date.now() < deadline && !windowClosed) {
			...
			if (stage === "мессенджер виден") {
				...
				console.log("");
				console.log(
					"Мессенджер отрендерился. Закрываю окно, чтобы профиль лёг на диск.",
				);
				break;
			}
			await sleep(POLL_INTERVAL_MS);
...
export async function main() {
	...
	if (!phase1.rendered && !phase1.windowClosed) {
		console.log("");
		console.log(
			`ТАЙМАУТ: за ${WAIT_MINUTES} мин мессенджер так и не отрендерился.`,
		);
	...
	if (result.status !== "MESSENGER_RENDERED") {
		...
		console.log(
			"  - Авито не принял сессию с этого IP (нужен обычный IP клиники);",
		);
		console.log(
			"  - AVITO_USER_AGENT не совпадает с браузером, откуда переехали куки;",
		);
		console.log(
			"  - вёрстка сменилась, и selectors.json устарел (тогда — discover.mjs).",
		);
		return 1;
	}
	...
	if (!selectors) {
		console.log("");
		console.log(
			"ВАЖНО: проверка шла по ЗАПАСНЫМ признакам, selectors.json ещё нет.",
		);
		console.log(
			`Число «элементов» ${result.chats} — это совпадения широкого OR-списка,`,
		);
		console.log("а не количество чатов. Следующий шаг: node src/discover.mjs");
	} else if (result.chats === 0) {
		console.log("");
		console.log(
			"Сессия жива, но чатов ноль. Либо переписок действительно нет,",
		);
		console.log(
			"либо chatItem в selectors.json больше не совпадает с вёрсткой.",
		);
	}

	console.log("");
	console.log(
		"Вход подтверждён. Профиль сохранён и годен для headless-рантайма.",
	);
	return 0;
}
