...
export function loadSelectors(root) {
	...
	if (missing.length) {
		throw new Error(
			`В selectors.json не заполнены обязательные ключи: ${missing.join(", ")}`,
		);
	}
	return parsed;
}
...
export async function checkSession(ctx, selectors) {
	...
	if (LOGIN_URL_MARKERS.some((re) => re.test(url))) {
		return {
			...
			url,
			chats: null,
			detail: "редирект на страницу входа",
		};
	}
	...
	try {
		...
		return {
			...
			chats: null,
			detail:
				`контейнер списка чатов (${selectors.chatList}) не отрендерился за 15 с — ` +
