...
function probePage({ sampleCount, sampleLen, fallbackItem }) {
  const CSS_ESCAPE = (value) => (window.CSS && CSS.escape
    ? CSS.escape(value)
  ...
  const selectorsFor = (el) => {
    ...
    for (const cls of el.classList) {
      const stem = cls.split(/[-_]/).filter((p) => !/^[a-zA-Z0-9]{5,}$/.test(p) || /^[a-z]+$/.test(p));
      if (stem.length >= 2) out.push(`[class*="${stem.slice(0, 2).join('-')}"]`);
    }
    return out;
  };
  ...
  const parents = new Set(itemNodes.map((el) => el.parentElement).filter(Boolean));
  ...
  if (scopes.length) {
    ...
    for (const item of scopes) {
      for (const el of item.querySelectorAll('*')) {
        ...
        if (/unread|badge|counter/i.test(marker) || /unread|badge/i.test(el.className)) {
          badges.push(el);
...
function probeDialog({ sampleCount, sampleLen }) {
  ...
  const textNodes = [...document.querySelectorAll('div, li, article')]
    .filter((el) => {
      const text = (el.textContent || '').trim();
  ...
  const feed = [...byParent.entries()].sort((a, b) => b[1].length - a[1].length)[0];
  ...
  for (const bubble of bubbles.slice(0, 12)) {
    for (const selector of attrSelectors(bubble)) {
      ...
      if (matched <= 0 || matched >= bubbles.length) {
        ...
        if (matched === 0 || matched === bubbles.length) {
          splits.push({ selector, matched, total: bubbles.length, usable: false,
                        why: matched === 0 ? 'не совпал ни с одним пузырём'
                                           : 'совпал со ВСЕМИ пузырями' });
        }
        continue;
      }
      splits.push({ selector, matched, total: bubbles.length, usable: true,
  ...
  report.keys.direction = splits.sort((a, b) => {
  ...
  if (field) {
    report.flags.inputKind = field.isContentEditable ? 'contenteditable' : 'textarea';
    report.flags.inputTag = field.tagName.toLowerCase();
  } else {
    report.notes.push('поле ввода не найдено: диалог открыт? возможно, чат в архиве');
  }
  ...
  if (!buttons.length) {
    report.notes.push('кнопка отправки не опознана — вероятно, отправка по Enter');
  }
...
function printCandidates(title, candidates, { required = false } = {}) {
  ...
  for (const item of candidates) {
    const scope = item.scopes > 1 ? `, найден в ${item.scopesHit} из ${item.scopes}` : '';
    console.log(`  ${item.selector}`);
    console.log(`      совпадений ${item.total}${scope}`);
    if (item.samples?.length) {
      console.log(`      примеры: ${item.samples.filter(Boolean).join(' | ') || '(без текста)'}`);
    }
  }
...
function printDirection(candidates) {
  ...
  console.log('  Без него бот примет свой ответ за вопрос пациента и уйдёт в самопереписку.');
  if (!candidates || !candidates.length) {
    console.log('  кандидатов не найдено — заполнять outgoingMark/incomingMark руками');
    return null;
  }
  let best = null;
  for (const item of candidates) {
  ...
  if (!best) {
    ...
    console.log('  признак, совпадающий со всеми пузырями или ни с одним, направление');
...
async function sessionAlive(page) {
  ...
  if (!chats) {
    return { alive: false, url,
             detail: 'ни одного элемента чата по широким запасным селекторам — ' +
...
async function main() {
  ...
  try {
    ...
    if (!health.alive) {
      ...
      console.error('  Кандидатов со страницы логина не выдаём — они были бы мусором.');
      console.error('  Нужен разовый вход руками: npm run login');
      return 1;
    }
    console.log(`Сессия жива: ${health.detail}`);
    console.log(`Файл selectors.json: ${current ? 'есть, сверяйтесь с ним' : 'отсутствует'}`);

    const list = await page.evaluate(probePage, {
      sampleCount: SAMPLES, sampleLen: SAMPLE_LEN, fallbackItem: FALLBACK_CHAT_ITEM,
    });

    console.log('\n================ СПИСОК ЧАТОВ ================');
    console.log(`Элементов чата на странице: ${list.counts.chatItemsSeen}`);
    const draft = {};
    draft.chatList = printCandidates('chatList', list.keys.chatList, { required: true });
    draft.chatItem = printCandidates('chatItem', list.keys.chatItem, { required: true });
    draft.unreadMark = printCandidates('unreadMark', list.keys.unreadMark, { required: true });
    draft.counterpartyName = printCandidates('counterpartyName', list.keys.counterpartyName);
    ...
    for (let i = 0; i < total; i += 1) {
      if (!draft.unreadMark) { opened = i; break; }
      if ((await items.nth(i).locator(draft.unreadMark).count()) === 0) { opened = i; break; }
    }
    if (opened < 0) {
      console.log('\nВсе диалоги непрочитаны — не открываем ни один, чтобы не пометить');
      console.log('их прочитанными. Запустите разведку позже или после ответа администратора.');
    ...
    const dialog = await page.evaluate(probeDialog, {
      sampleCount: SAMPLES, sampleLen: SAMPLE_LEN,
    });

    console.log('\n================ ОТКРЫТЫЙ ДИАЛОГ ================');
    console.log(`Пузырей в ленте: ${dialog.counts.bubblesSeen}`);
    draft.messageBubble = printCandidates('messageBubble', dialog.keys.messageBubble,
                                          { required: true });
    const direction = printDirection(dialog.keys.direction);
    draft.input = printCandidates('input', dialog.keys.input, { required: true });
    ...
    if (dialog.keys.messageId?.length) {
      console.log(`messageId: атрибуты на пузыре — ${dialog.keys.messageId.join(', ')}`);
      draft.messageId = dialog.keys.messageId[0];
    } else {
      console.log('messageId: стабильного id в DOM нет — дедуп пойдёт по отпечатку');
    }
    ...
    if (match) {
      ...
      console.log(`chatIdFromUrl: шаблон не совпал с ${page.url()} — правьте вручную`);
    }

    for (const note of [...list.notes, ...dialog.notes]) console.log(`ЗАМЕЧАНИЕ: ${note}`);
...
function writeDraft(draft, list, direction) {
  ...
  const missing = ['chatList', 'chatItem', 'unreadMark', 'messageBubble', 'input',
                   'sendButton'].filter((key) => !draft[key]);
  const payload = {
    _: 'ЧЕРНОВИК разведки. Проверьте каждую строку и скопируйте в selectors.json ' +
  ...
  if (!direction) {
    console.log('НЕ НАЙДЕН признак направления — без него запускать поллер нельзя.');
  }
  console.log('Это черновик, а не selectors.json. Скопируйте проверенные строки сами.');
}
